...
async fn handler_resolves_query_returns_no_error() {
    ...
    let handler = DnsFilterHandler::new(vec![], Arc::new(chain));
...
async fn handler_middleware_short_circuit_returns_refused() {
    ...
    let handler =
        DnsFilterHandler::new(vec![sc_mw.clone() as Arc<dyn Middleware>], Arc::new(chain));
...
async fn handler_after_request_called_on_all_middlewares() {
    ...
    let handler = DnsFilterHandler::new(
