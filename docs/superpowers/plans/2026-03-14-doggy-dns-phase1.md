# doggy-dns Phase 1 Implementation Plan
...
**Architecture:** Hybrid two-layer pipeline — a custom `RequestHandler` for outer middleware (logging, metrics), delegating to an `AuthorityChain` of `ZoneHandler` implementations (NacosAuthority → NativeAuthority → ForwardAuthority). Each authority uses `LookupControlFlow::Skip` to decline and pass to the next.
...
**Spec:** `docs/superpowers/specs/2026-03-14-doggy-dns-phase1-design.md`
...
doggy-dns/
  ...
  config/doggy-dns.toml                        # Example config (new)

  crates/doggy-dns-core/Cargo.toml             # Core crate manifest (new)
  crates/doggy-dns-core/src/lib.rs             # Core re-exports (new)
  crates/doggy-dns-core/src/config.rs          # TOML config structs (new)
  crates/doggy-dns-core/src/server.rs          # Server setup UDP/TCP/TLS/HTTPS (new)
  crates/doggy-dns-core/src/handler.rs         # DoggyDnsHandler: RequestHandler impl (new)
  crates/doggy-dns-core/src/middleware/mod.rs   # Middleware trait + chain (new)
  crates/doggy-dns-core/src/middleware/logging.rs  # Logging middleware (new)
  crates/doggy-dns-core/src/middleware/metrics.rs  # Metrics middleware (new)

  crates/doggy-dns-plugin/Cargo.toml           # Plugin crate manifest (new)
  crates/doggy-dns-plugin/src/lib.rs           # Plugin re-exports (new)
  crates/doggy-dns-plugin/src/plugin.rs        # Middleware Plugin trait (new)
  crates/doggy-dns-plugin/src/authority.rs     # AuthorityPlugin factory trait (new)
  crates/doggy-dns-plugin/src/authority_chain.rs  # AuthorityChain: Vec<Box<dyn ZoneHandler>> (new)
  crates/doggy-dns-plugin/src/registry.rs      # Plugin registry from config (new)
  crates/doggy-dns-plugin/src/builtin/mod.rs   # Built-in plugin re-exports (new)
  crates/doggy-dns-plugin/src/builtin/native.rs  # NativeAuthority (new)
  crates/doggy-dns-plugin/src/builtin/forward.rs     # ForwardAuthority (new)

  crates/doggy-dns-nacos/Cargo.toml            # Nacos crate manifest (new)
  crates/doggy-dns-nacos/src/lib.rs            # Nacos re-exports (new)
  crates/doggy-dns-nacos/src/authority.rs      # NacosAuthority: ZoneHandler impl (new)
  crates/doggy-dns-nacos/src/watcher.rs        # Nacos service subscription + cache (new)
  crates/doggy-dns-nacos/src/mapping.rs        # ServiceInstance → DNS Record (new)
  crates/doggy-dns-nacos/src/config_watcher.rs # Nacos config listener for hot-reload (new)
```
...
- Create: `crates/doggy-dns-core/Cargo.toml`
- Create: `crates/doggy-dns-plugin/Cargo.toml`
- Create: `crates/doggy-dns-nacos/Cargo.toml`
- Create: `src/main.rs`
- Create: `crates/doggy-dns-core/src/lib.rs`
- Create: `crates/doggy-dns-plugin/src/lib.rs`
- Create: `crates/doggy-dns-nacos/src/lib.rs`
...
resolver = "2"
members = [
    "crates/doggy-dns-core",
    "crates/doggy-dns-plugin",
    "crates/doggy-dns-nacos",
]
...
[dependencies]
doggy-dns-core = { path = "crates/doggy-dns-core" }
doggy-dns-plugin = { path = "crates/doggy-dns-plugin" }
doggy-dns-nacos = { path = "crates/doggy-dns-nacos" }
...
- [ ] **Step 2: Create doggy-dns-plugin Cargo.toml** (fewest deps, foundation crate)

```toml
[package]
name = "doggy-dns-plugin"
version = "0.1.0"
edition = "2024"
license = "Apache-2.0"
...
- [ ] **Step 3: Create doggy-dns-core Cargo.toml**
...
[dependencies]
doggy-dns-plugin = { path = "../doggy-dns-plugin" }
hickory-server = { workspace = true }
hickory-proto = { workspace = true }
serde = { workspace = true }
toml = { workspace = true }
...
- [ ] **Step 4: Create doggy-dns-nacos Cargo.toml**

```toml
[package]
name = "doggy-dns-nacos"
version = "0.1.0"
edition = "2024"
license = "Apache-2.0"
...
`crates/doggy-dns-core/src/lib.rs`:
```rust
pub mod config;
```

`crates/doggy-dns-plugin/src/lib.rs`:
```rust
pub mod plugin;
pub mod authority;
...
`crates/doggy-dns-nacos/src/lib.rs`:
```rust
pub mod authority;
pub mod watcher;
pub mod mapping;
...
Files:
- `crates/doggy-dns-core/src/config.rs`
- `crates/doggy-dns-plugin/src/plugin.rs`
- `crates/doggy-dns-plugin/src/authority.rs`
- `crates/doggy-dns-plugin/src/authority_chain.rs`
- `crates/doggy-dns-plugin/src/registry.rs`
- `crates/doggy-dns-plugin/src/builtin/mod.rs`
- `crates/doggy-dns-plugin/src/builtin/native.rs`
- `crates/doggy-dns-plugin/src/builtin/forward.rs`
- `crates/doggy-dns-nacos/src/authority.rs`
- `crates/doggy-dns-nacos/src/watcher.rs`
- `crates/doggy-dns-nacos/src/mapping.rs`
- `crates/doggy-dns-nacos/src/config_watcher.rs`
...
**Files:**
- Create: `crates/doggy-dns-core/src/config.rs`
- Create: `crates/doggy-dns-core/tests/config_test.rs`

- [ ] **Step 1: Write failing tests for config parsing**

`crates/doggy-dns-core/tests/config_test.rs`:
```rust
use doggy_dns_core::config::{DoggyDnsConfig, PluginKind};
...
"#;
    ...
    let config: DoggyDnsConfig = toml::from_str(toml_str).unwrap();
    ...
    let native = &config.plugins[1];
    assert_eq!(native.kind, PluginKind::Native);
    assert_eq!(native.cache_size, 256);
    assert_eq!(native.min_ttl, 10);
    assert_eq!(native.max_ttl, 300);
...
"#;
    ...
    let result: Result<DoggyDnsConfig, _> = toml::from_str(toml_str);
...
Run: `cargo test -p doggy-dns-core --test config_test`
...
pub struct PluginConfig {
    ...
    pub ttl: u32,
    // Resolver-specific (native / forward)
...
pub fn load_config(path: &str) -> anyhow::Result<DoggyDnsConfig> {
    let content = std::fs::read_to_string(path)?;
    let config: DoggyDnsConfig = toml::from_str(&content)?;
    validate_config(&config)?;
    Ok(config)
}

fn validate_config(config: &DoggyDnsConfig) -> anyhow::Result<()> {
...
**Files:**
- Create: `crates/doggy-dns-plugin/src/plugin.rs`
- Create: `crates/doggy-dns-plugin/src/authority.rs`
...
Run: `cargo check -p doggy-dns-plugin`
...
**Files:**
- Create: `crates/doggy-dns-plugin/src/authority_chain.rs`
- Create: `crates/doggy-dns-plugin/tests/authority_chain_test.rs`
...
Run: `cargo test -p doggy-dns-plugin --test authority_chain_test`
...
**Files:**
- Create: `crates/doggy-dns-plugin/src/builtin/forward.rs`
- Create: `crates/doggy-dns-plugin/src/builtin/mod.rs`
- Create: `crates/doggy-dns-plugin/tests/forward_test.rs`
...
Run: `cargo test -p doggy-dns-plugin --test forward_test`
...
### Task 6: NativeAuthority (built-in)

**Files:**
- Create: `crates/doggy-dns-plugin/src/builtin/native.rs`
- Create: `crates/doggy-dns-plugin/tests/native_test.rs`

- [ ] **Step 1: Write failing test for NativeAuthority**
...
Run: `cargo test -p doggy-dns-plugin --test native_test`
Expected: FAIL — `NativeAuthority` does not exist

- [ ] **Step 3: Implement NativeAuthority**
...
#[async_trait]
impl ZoneHandler for NativeAuthority {
    fn zone_type(&self) -> ZoneType {
        ZoneType::Forward
    }
...
```bash
git add -A
git commit -m "feat: add NativeAuthority wrapping system /etc/resolv.conf"
```
...
**Files:**
- Create: `crates/doggy-dns-nacos/src/mapping.rs`
- Create: `crates/doggy-dns-nacos/tests/mapping_test.rs`

- [ ] **Step 1: Write failing tests for mapping**

`crates/doggy-dns-nacos/tests/mapping_test.rs`:
```rust
use doggy_dns_nacos::mapping::{parse_dns_name, DnsNameParts, to_dns_records};
use hickory_proto::rr::{Name, RecordType};
...
Run: `cargo test -p doggy-dns-nacos --test mapping_test`
...
**Files:**
- Create: `crates/doggy-dns-nacos/src/watcher.rs`
...
Run: `cargo check -p doggy-dns-nacos`
...
**Files:**
- Create: `crates/doggy-dns-nacos/src/authority.rs`
- Create: `crates/doggy-dns-nacos/tests/authority_test.rs`
...
Run: `cargo test -p doggy-dns-nacos --test authority_test`
...
**Files:**
- Create: `crates/doggy-dns-core/src/middleware/mod.rs`
- Create: `crates/doggy-dns-core/src/middleware/logging.rs`
- Create: `crates/doggy-dns-core/src/middleware/metrics.rs`

- [ ] **Step 1: Create middleware mod.rs**

`crates/doggy-dns-core/src/middleware/mod.rs`:
```rust
pub mod logging;
pub mod metrics;
```
...
`crates/doggy-dns-core/src/middleware/logging.rs`:
```rust
use async_trait::async_trait;
...
`crates/doggy-dns-core/src/middleware/metrics.rs`:
```rust
use async_trait::async_trait;
use doggy_dns_plugin::{Middleware, MiddlewareAction};
use hickory_server::server::Request;
...
- [ ] **Step 4: Update doggy-dns-core/src/lib.rs**
...
Run: `cargo check -p doggy-dns-core`
Expected: Compiles
...
### Task 11: DoggyDnsHandler (RequestHandler)

**Files:**
- Create: `crates/doggy-dns-core/src/handler.rs`
- Create: `crates/doggy-dns-core/tests/handler_test.rs`

- [ ] **Step 1: Implement DoggyDnsHandler**

`crates/doggy-dns-core/src/handler.rs`:
```rust
use async_trait::async_trait;
use doggy_dns_plugin::authority_chain::AuthorityChain;
use doggy_dns_plugin::{Middleware, MiddlewareAction};
...
```bash
git add -A
git commit -m "feat: add DoggyDnsHandler implementing RequestHandler"
```
...
**Files:**
- Create: `crates/doggy-dns-core/src/server.rs`

- [ ] **Step 1: Implement server setup**

`crates/doggy-dns-core/src/server.rs`:
```rust
use crate::config::{DoggyDnsConfig, HttpsConfig, TlsConfig};
...
- [ ] **Step 2: Add rustls-pemfile dependency to doggy-dns-core**

Add to `crates/doggy-dns-core/Cargo.toml` under `[dependencies]`:
```toml
rustls-pemfile = "2"
```
...
**Files:**
- Create: `crates/doggy-dns-core/tests/middleware_test.rs`
- Create: `crates/doggy-dns-core/tests/handler_test.rs`
...
Run: `cargo test -p doggy-dns-core`
...
```rust
use anyhow::{Context, Result};
use doggy_dns_core::config::{load_config, DoggyDnsConfig, PluginKind};
...
async fn build_authority_chain(config: &DoggyDnsConfig) -> Result<AuthorityChain> {
    ...
    for plugin in &config.plugins {
        ...
        match plugin.kind {
            ...
            PluginKind::Native => {
                ...
                handlers.push(Arc::new(authority));
                tracing::info!("loaded native plugin");
            }
            PluginKind::Forward => {
...
**Files:**
- Create: `crates/doggy-dns-nacos/src/config_watcher.rs`
...
| 6 | NativeAuthority + test | `feat: add NativeAuthority` |
...
| 11 | DoggyDnsHandler | `feat: add DoggyDnsHandler` |
